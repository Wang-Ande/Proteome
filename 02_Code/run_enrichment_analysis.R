run_enrichment_analysis <- function(data, OrgDb, logFC ,dir) {
  ...
  down_genes <- subset(data, logFC < -logFC)
  ...
  p2 <- dotplot(kk_down,showCategory = 15)
  print(p2)
  dev.off()

  # up-regulated genes
  up_genes <- subset(data, logFC > logFC)
  ...
  p4 <- dotplot(kk_up,showCategory = 15)
  print(p4)
  dev.off()
}
